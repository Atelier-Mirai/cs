...
//abstract{
JavaScriptの基本的な文法を学びつつ、少しずつコードを追加していくことで、じゃんけんゲームを作っていきます。
//}

== 初めてのJavaScript

=== 初めてのJavaScript
初めてのJavaScriptのプログラムを書き始めていきましょう。

@<file>{janken00.js} というファイルを作り、次のように書きましょう。 @<fn>{1}
//footnote[1][「開発環境の準備」の章で、index.html を作りました。同様の手順で、janken00.js というファイルを追加して、これを編集していきます。]
 ...
 * @<code>{let player_name     // (JavaScriptでは一般的ではありませんが) _ も使えます}
...
JavaScriptはプログラミング言語ですので、様々な計算を行うことができます。「@<B>{演算子}」とは聞きなれない用語かもしれませんが、平たく言えば、@<m>$7 + 5$ の @<m>$+$ や、@<m>$18 - 3$ の @<m>$-$ のように、どのような計算(演算)を行うかを示すための記号が「@<B>{演算子}」です。
...
JavaScriptでも、もちろんこのような値を返す関数を定義することもできますし、また、値を返すことが目的ではなく、何か処理を行うことが目的の関数を作ることもできます。 @<fn>{fn-alert}
...
//sideimage[add32][60mm][sep=5mm,side=R]{
...
そして、JavaScriptの関数は「第一級オブジェクト」と呼ばれ、関数を変数に代入できる！性質を持ちます。5と27を足し算した結果である32を変数に代入するのではなく、「足し算する」という「機能・働き・関数」そのものを変数に代入できるという、特筆すべき性質を持っています。
...
//blankline
またより詳しい説明が、公式サイト ＭＤＮ @<href>{https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Functions/Arrow_functions,アロー関数式} にございますので、ご興味を持った方はお読みになってください。
...
辞典ですので、きっちりと書かれておりますが、少し難しく感じるかもしれませんが、使い方は簡単です。

練習用の簡単なHTMLを準備して、実験して見ましょう。ファイル名は @<file>{dom.html}としましょう。
...
この JavaScript を HTML から読み込む為に、最初に書いた @<file>{dom.html} の @<code>{<head>}タグの中に @<code>{<script src="dom.js" defer></script>} の一行を追加します。
...
//list[][janken10.js][file=source/janken10.js, 1]{
//}

一行ずつ解説していきます。

2行目の @<code>{const playButton = document.getElementById("play")} についてです。まず @<code>{document.getElementById("play")} で、HTML に書いた 開始ボタン要素を取得します（DOM という仕組みにより、JavaScript から HTML要素を操作できました）。そして取得した 開始ボタン要素をプログラムの中で扱いやすくするために @<code>{playButton} という変数を宣言し、代入しています。

3行目の @<code>{playButton.addEventListener("click", jankenHandler)} です。まず @<code>{playButton} にイベントリスナを追加します(@<code>{addEventListener})。
...
6行目から8行目は、@<code>{jankenHandler} という関数を定義しています。
...
それでは、@<code>{janken10.js} を保存して、ブラウザを再読み込みしてみましょう。
...
@<file>{janken00.js} から @<file>{janken10.js} へとファイル名を変更しています。

//blankline
呼び出し元となる @<file>{index.html} のコードも

//list[][index.html][]{
<!-- JavaScriptの読み込み -->
<script src="janken@<b>{00}.js" defer></script>
//}
...
//list[][][]{
<!-- JavaScriptの読み込み -->
<script src="janken@<b>{10}.js" defer></script>
//}
...
プログラミング言語には、同じ処理を繰り返したいときの為の専用の構文(書き方)が用意されています。@<code>{for文} を使い、次のように書いてみましょう。
...
@<file>{for}文の実行の流れは次のようになります。

 1. 初期化式 で変数の宣言
 2. 条件式 の評価結果がtrue(真・成り立つ)なら次のステップへ、false(偽・成り立たない)なら終了
 3. 実行する文 を実行
 4. 増分式 で変数を更新
 5. ステップ2へ戻る
...
@<code>{for文} の例として、1から10までの値を合計して、その結果を出力するプログラムを作ってみましょう。 今まで作って来た @<file>{janken10.js} の最後に追加してみましょう。
...
「@<B>{コンソール}」という新しい言葉が登場しました。「操作盤」という意味をもつ英単語です。
@<code>{console.log()} という命令(メソッド、関数)を使って、ブラウザのコンソール画面に結果を表示できます。

//sideimage[total55][70mm][sep=5mm,side=R]{
Firefoxでは、次のようにすると、コンソール画面と呼ばれる画面を表示できます。

 1. ブラウザ画面を右クリック
 2. 「調査」をクリック
 3. 「コンソール」パネルをクリック

 コンソール画面に @<code>{55} と出力されています。
//}
...
条件式の評価結果が @<code>{@<ruby>{true,トゥルー}} (@<ruby>{真,しん}・成り立つ) @<fn>{fn-true} であるならば、実行する文が実行されます。

//footnote[fn-true][反対語は@<code>{@<ruby>{false,フォールス}} (@<ruby>{偽,ぎ}・成り立たない)です。]
...
@<code>{if文} と @<code>{else if文} では、条件に一致した場合の処理をブロック内に書いていました。 いずれの条件にも一致しなかった場合の処理は、 @<code>{else文} を使うことで書けます。
...
そのために、@<code>{if文} を使って、以下のように書いてみましょう。@<small>{(キャッシュの影響を避けるために、ファイル名を @<file>{janken@<B>{30}.js}にしています。 @<file>{index.html} でも @<code>{<script src="janken@<B>{30}.js" defer></script>)} と更新しましょう。}


//list[][janken30.js][file=source/janken30.js, 1]{
//}

4行目までは、前回と一緒です。

7行目と8行目で、プレイヤーが入力した手を取得します。
@<file>{index.html}には @<code>{<input type="number" id="player_hand_type" value="0">} とプレイヤーが自分の手（グーなら0、チョキなら1、パーなら2）を入力するための「入力枠」要素を設けてあります。

この「入力枠」にどのような手(数値)が入力されたのかをJavaScriptで取得し、プレイヤーの手(数値)に依って、勝敗を判定、結果を表示するようにします。

//blankline
まず @<code>{document.getElementById("player_hand_type")}と書き、このプレイヤーの入力枠要素自体をJavaScriptで取得します。取得した入力枠に名前があると扱いやすいので、@<code>{const inputBox} と書き @<code>{inputBox}という名前の変数を用意します。そして @<code>{const inputBox = document.getElementById("player_hand_type")} と @<code>{代入演算子 @<ruby>{=,イコール}} を使って、取得したプレイヤーの入力枠要素を @<code>{inputBox}変数に代入します。これで @<code>{inputBox}変数がプレイヤーの入力枠を指すようになりましたので、 @<code>{document.getElementById("player_hand_type")} と書く代わりに、短く @<code>{inputBox} という名称（変数名）で使うことができます。

//blankline
@<code>{inputBox}から プレイヤーが枠に入力した値を取得するるには、@<code>{@<ruby>{value,バリュー}}メソッドを使います。@<code>{inputBox.value} と書くとプレイヤーが枠に入力した値を得ることができます。

@<code>{Number} は、文字列としての @<code>{"0"} を数値としての @<code>{0} にする関数です。一般にプログラミング言語では、文字列としての @<code>{"0"} と、整数値としての @<code>{0} とは異なります(ちょうど、漢数字の @<code>{〇}と、数値の @<code>{0} が異なるようにです)。整数値に変換した結果を、@<code>{let player = }と書いて、代入しています。

以上で、プレイヤーが入力した手を取得することができるようになりました。


//blankline
11 行目の、@<code>{let computer = 0} は、コンピュータの手を設定しています。

0はグー、1はチョキ、2はパー という約束でした。今のところ、コンピュータは必ず「グー」を出すことにして作っていきましょう。

//blankline
プレイヤーの手とコンピュータの手が出そろったので、先ほど学んだ @<code>{if文}を使って、勝ち負けの判定を行います。
...
@<code>{1}を入力した場合、
@<code>{2}を入力した場合、
それぞれで、正しくメッセージが表示されるか確認してみてください。
(メッセージが表示されたらブラウザを再読み込みして違う手を入力してください。)
...
//list[][janken40.js][file=source/janken40.js, 1]{
//}
...
//list[][janken60.js][file=source/janken60.js, 1]{
// じゃんけんの勝ち負けの結果を表示する関数
function jankenHandler(event) {
  // judge は、
  // プレイヤーとコンピュータの手を与えると、
 ...
 * 勝敗判定関数の作成。
 * 9通りのif文からアルゴリズムの見直しを行い、わずか一行の判定式に改良。
