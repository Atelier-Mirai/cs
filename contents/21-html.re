...
PWA(Progressive Web App)とは、ブラウザ上で動作するウェブアプリケーションのことで、次のような特徴があります。

 * レスポンシブデザインで、あらゆるデバイスに最適化されている。
 * オフラインでの利用 - Service Workerを使ってネットワークが接続されていなくても一部の機能を利用できる。
 * プッシュ通知 - ユーザーの許可を得てプッシュ通知を受け取ることができる。
 * ホーム画面への追加 - ブラウザからアプリのようにホーム画面に追加できる。
 * 高速な起動と読み込み - キャッシュ機能などにより高速な表示が可能である。

つまり、PWAは従来のウェブアプリケーションの課題を解決し、ネイティブアプリに迫る機能性と使いやすさを実現したものです。モバイルユーザーにとって、インストールが簡単で軽量なウェブアプリとして利用できるのが大きな特徴です。
...
いま作成した @<file>{index.html} をブラウザで開くと次のようになります。

//image[janken][現状(左) 完成形(右)][width=75%]

左側の現状は、少し寂しいですが、次章では右側のように綺麗に仕上げていきます。完成を目指して一緒に学んでいきましょう。
