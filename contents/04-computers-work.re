...
//sideimage[computer][55mm][sep=5mm,side=R]{
  ...
  IT用語辞典 @<fn>{1} から抜粋しつつ、「コンピュータ」の定義を紹介するとともに、その構成要素の詳細を見ていきましょう。
//}
//vspace[latex][2mm]
...
=== コンピュータの分類
一般的にコンピュータとみなされる機器には、個人向けの汎用コンピュータである「パーソナルコンピュータ」（PC / パソコン）や、企業や官公庁などの情報システムで用いられる大規模・高性能コンピュータである「サーバ」や「メインフレーム」、科学技術計算などに用いる超高性能コンピュータである「スーパーコンピュータ」 @<fn>{supakon}などがある。

//footnote[supakon][普通のパソコンの処理能力は80GFLOPSです。つまり毎秒800億回=0.08兆回の浮動小数点数の計算が出来ます。理化学研究所の「富岳」は440PLOPS(毎秒44京回=440000兆回)の演算が可能です。つまり普通のパソコンの550万倍の性能があります。]
...
=== アプリケーションソフト
#@# OSの機能を利用し、OSの上で動作するソフトウェアである。アプリケーション開発者は、OSが提供する機能を利用することにより、効率的に開発でき、操作性も統一できる。また、ハードウェア仕様の細かな違いはOSが吸収するため、あるOS向けに開発されたソフトウェアは、基本的にはそのOSが動作するどんなコンピュータでも利用できる。
#@#
 ...
 * 1,000 @<ruby>{m,メートル} = 1 @<ruby>{km,キロメートル}
 * 1 @<ruby>{mm,ミリメートル} = 1 / 1,000 @<ruby>{m,メートル}
